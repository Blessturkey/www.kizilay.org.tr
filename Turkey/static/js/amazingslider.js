	...
	$.fn.amazingslider = function (options) {
		...
		AmazingSlider.prototype = {
			...
			readTags: function () {
				...
				$(".amazingslider-slides", this.container).find("li").each(function () {
					...
					if (img.length > 0) {
						...
						for (var i = 0; i < lazytags.length; i++)
							if (img.data(lazytags[i]) &&
								img.data(lazytags[i]).length > 0) {
						...
						var lightbox =
							img.parent() && img.parent().is("a") ? img.parent().hasClass("html5lightbox") : false;
						...
						var elem = new Array(instance.elemArray.length, src, title, description, link, target, video, "", lightbox, lightboxwidth, lightboxheight, dataWebm, dataOgg, texteffect,
							duration, button, mediaType, alt);
				...
				$(".amazingslider-thumbnails", this.container).find("li").each(function (index) {
					...
					if (img.length > 0 && instance.elemArray.length > index) {
						...
						var lazytags = instance.options.lazyloadtags.split(",");
						for (var i = 0; i < lazytags.length; i++)
							if (img.data(lazytags[i]) && img.data(lazytags[i]).length >
								0) {
								src = img.data(lazytags[i]);
								break
							}
			...
			initHtml5Lightbox: function () {
				...
				if (this.lightboxArray.length > 0) {
					...
					for (i = 0; i < this.lightboxArray.length; i++) this.html5Lightbox.addItem(this.lightboxArray[i][ELEM_LINK], this.lightboxArray[i][ELEM_TITLE], this.options.lightboxnogroup ? "" : "amazingslider" + this.id, this.lightboxArray[i][ELEM_LIGHTBOXWIDTH], this.lightboxArray[i][ELEM_LIGHTBOXHEIGHT], this.lightboxArray[i][ELEM_WEBM], this.lightboxArray[i][ELEM_OGG], this.lightboxArray[i][ELEM_THUMBNAIL],
						this.lightboxArray[i][ELEM_DESCRIPTION], this.lightboxArray[i][ELEM_VIDEOTYPE] == TYPE_IFRAME ? 9 : -1);
					var instance = this;
			...
			createRibbon: function () {
				...
				$(".amazingslider-ribbon-" + this.id, this.container).html("<img src='" + this.options.ribbonimage +
			...
			createWm: function () {
				...
				if (this.options.watermarkstyle == "text") html += this.options.wmtext;
					...
					if (watermarkimage.substring(0, 7).toLowerCase() != "http://" && watermarkimage.substring(0,
							8).toLowerCase() != "https://") watermarkimage = this.options.skinsfolder + watermarkimage;
			...
			showSocial: function () {
				...
				var instance = this;
				var socialCode = '<div class="amazingslider-social-' + this.id + '" style="display:' + (this.options.socialmode ==
					"mouseover" ? "none" : "block") + ";" + this.options.socialposition + '">';
				...
				if (this.options.showfacebook) socialCode += '<div class="amazingslider-social-btn' +
					(this.options.socialrotateeffect ? " amazingslider-rotate" : "") + ' amazingslider-social-facebook" style="' + socialBtnCSS + '"><div class="mh-icon-facebook" style="' + socialCSS + 'background-color:#3b5998;"></div></div>';
				...
				if (this.options.showpinterest) socialCode +=
					'<div class="amazingslider-social-btn' + (this.options.socialrotateeffect ? " amazingslider-rotate" : "") + ' amazingslider-social-pinterest" style="' + socialBtnCSS + '"><div class="mh-icon-pinterest" style="' + socialCSS + 'background-color:#c92228;"></div></div>';
				socialCode += '<div style="clear:both;"></div></div>';
				...
				$(".amazingslider-social-btn", this.container).click(function () {
					...
					else if ($(this).hasClass("amazingslider-social-twitter")) window.open("https://twitter.com/share?url=" + encodeURIComponent(shareUrl) + "&text=" + encodeURIComponent(shareTitle),
						"_blank");
			...
			createMarkup: function () {
				this.$wrapper = jQuery("" + "<div class='amazingslider-wrapper-" + this.id + "'>" + "<div class='amazingslider-background-image-" + this.id + "'></div>" + "<div class='amazingslider-slider-" + this.id + "'>" + "<div class='amazingslider-bottom-shadow-" + this.id + "'></div>" + "<div class='amazingslider-box-" + this.id + "'>" + "<div class='amazingslider-swipe-box-" +
					...
					this.id + "'></div>" + "<div class='amazingslider-arrow-right-" + this.id + "'></div>" + "</div>" + "<div class='amazingslider-nav-" + this.id + "'><div class='amazingslider-nav-container-" + this.id + "'></div></div>" + "</div>");
